...
bool Win32Window::Create(const std::wstring& title,
  ...
  RECT bounds;
  bounds.left = Scale(origin.x, scale_factor);
  bounds.top = Scale(origin.y, scale_factor);
  bounds.right = bounds.left + Scale(size.width, scale_factor);
  bounds.bottom = bounds.top + Scale(size.height, scale_factor);

  return Create(title, bounds);
}

bool Win32Window::Create(const std::wstring& title, const RECT& bounds) {
  Destroy();
