...
The public HTTPS media routes are:
...
They resolve to the corresponding application routes:
...
Media sharing and Discord Rich Presence use the clean HTTPS routes. On the
static GitHub Pages site, `404.html` validates a clean media path and forwards
it to the existing `open.html` bridge, which attempts the application URL and
keeps a manual **Open in MiruShin** button available.

The legacy bridge remains supported:

```text
https://mirushin.emp0ry.com/open.html?target=mirushin%3A%2F%2Fanilist%2Fanime%2F21
```

The static pages publish only generic MiruShin social metadata. Per-media
titles, descriptions, and posters are not generated.
