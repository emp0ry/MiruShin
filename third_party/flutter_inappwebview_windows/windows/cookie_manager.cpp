...
namespace flutter_inappwebview_plugin
{
    ...
    std::unique_ptr<flutter::MethodResult<flutter::EncodableValue>> result)
  {
    ...
    std::optional<uint64_t> webViewIdInt;
    std::optional<std::string> webViewIdString;
    const auto webViewIdIterator = arguments.find(make_fl_value("webViewId"));
    if (webViewIdIterator != arguments.end()) {
      const auto& webViewId = webViewIdIterator->second;
      if (const auto* id32 = std::get_if<int32_t>(&webViewId); id32 && *id32 >= 0) {
        webViewIdInt = static_cast<uint64_t>(*id32);
      }
      else if (const auto* id64 = std::get_if<int64_t>(&webViewId); id64 && *id64 >= 0) {
        webViewIdInt = static_cast<uint64_t>(*id64);
      }
      else if (const auto* idString = std::get_if<std::string>(&webViewId)) {
        webViewIdString = *idString;
      }
    }
    if (webViewIdInt.has_value() && plugin->inAppWebViewManager && map_contains(plugin->inAppWebViewManager->webViews, webViewIdInt.value())) {
  ...
  void CookieManager::setCookie(WebViewEnvironment* webViewEnvironment, InAppWebView* inAppWebView, const flutter::EncodableMap& map, std::function<void(const bool&)> completionHandler) const
  {
    ...
    auto targetWebView = inAppWebView && inAppWebView->webView ? inAppWebView->webView : webViewEnvironment->getWebView();
    if (!targetWebView) {
      if (completionHandler) {
        completionHandler(false);
      }
    ...
    auto hr = targetWebView->CallDevToolsProtocolMethod(L"Network.setCookie", utf8_to_wide(parameters.dump()).c_str(), Callback<ICoreWebView2CallDevToolsProtocolMethodCompletedHandler>(
  ...
  void CookieManager::getCookie(WebViewEnvironment* webViewEnvironment, InAppWebView* inAppWebView, const std::string& url, const std::string& name, std::function<void(const flutter::EncodableValue&)> completionHandler) const
  {
    ...
    auto hr = targetWebView->CallDevToolsProtocolMethod(L"Network.getCookies", utf8_to_wide(parameters.dump()).c_str(), Callback<ICoreWebView2CallDevToolsProtocolMethodCompletedHandler>(
  ...
  void CookieManager::deleteCookie(WebViewEnvironment* webViewEnvironment, InAppWebView* inAppWebView, const std::string& url, const std::string& name, const std::string& path, const std::optional<std::string>& domain, std::function<void(const bool&)> completionHandler) const
  {
    ...
    auto hr = targetWebView->CallDevToolsProtocolMethod(L"Network.deleteCookies", utf8_to_wide(parameters.dump()).c_str(), Callback<ICoreWebView2CallDevToolsProtocolMethodCompletedHandler>(
      [completionHandler](HRESULT errorCode, LPCWSTR returnObjectAsJson)
      {
        if (completionHandler) {
          completionHandler(succeededOrLog(errorCode));
