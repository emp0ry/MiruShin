...
namespace applinks
{
		...
		LPARAM lparam)
	{
		...
		if (message == WM_COPYDATA)
		{
			...
			if (cds != nullptr && cds->dwData == APPLINK_MSG_ID &&
				cds->lpData != nullptr && cds->cbData > 0 && cds->cbData <= 4097)
			{
				// Backported from app_links 7.1.2: only accept another process
				// running the same executable and never read beyond cbData.
				DWORD senderPid = 0;
